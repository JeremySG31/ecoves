...
const reportSchema = z.object({
  report: z.string().min(10, { message: 'El reporte debe tener al menos 10 caracteres.' }),
})

// --- Acción para Guardar Reporte ---
export async function saveReport(prevState: any, formData: FormData) {
  ...
  return { message: '¡Gracias! Tu reporte ha sido enviado y será revisado.', errors: null, success: true }
}
...
export async function registerForEvent(prevState: any, formData: FormData) {
    ...
    console.log(`Nuevo registro para ${validatedFields.data.eventName}:`, `Nombre: ${validatedFields.data.name}, Email: ${validatedFields.data.email}`);
